...
void Database::prepareDB(QString targetDir)
{
    ...
    QSqlQuery query(db);
    query.exec("drop table files"); //##### It seems that there is no table named "files", why need to drop it here.
    QString str = getSql(TypeCreate);

    if (!query.exec(str))   //##### If there is already a table named posotions in database.db, then the value query.exec(str) return will be false.
    {
        //创建一个表
...
//        rec.appendStringList(fileName, params);
        //#####
        rec.appendStringList(counter, fileName, params);
